/*
 * Library coff_header type test program
 *
...
#include "../libexe/libexe_coff_header.h"

#if defined( __GNUC__ ) && !defined( LIBEXE_DLL_IMPORT )

/* Tests the libexe_coff_header_initialize function
 * Returns 1 if successful or 0 if not
 */
int exe_test_coff_header_initialize(
     void )
{
	libcerror_error_t *error          = NULL;
	libexe_coff_header_t *coff_header = NULL;
	int result                        = 0;
	...
	/* Test regular cases
	 */
	result = libexe_coff_header_initialize(
	          &coff_header,
	          &error );
	...
	result = libexe_coff_header_free(
	          &coff_header,
	          &error );
	...
	coff_header = (libexe_coff_header_t *) 0x12345678UL;
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libexe_coff_header_initialize with malloc failing
		 */
	...
	for( test_number = 0;
	     ...
	     test_number++ )
	{
		/* Test libexe_coff_header_initialize with memset failing
		 */
...
/* Tests the libexe_coff_header_free function
 * Returns 1 if successful or 0 if not
 */
int exe_test_coff_header_free(
     void )
{
	libcerror_error_t *error = NULL;
	int result               = 0;
...
#endif
{
	...
	EXE_TEST_RUN(
	 "libexe_coff_header_initialize",
	 exe_test_coff_header_initialize );

	EXE_TEST_RUN(
	 "libexe_coff_header_free",
	 exe_test_coff_header_free );

	/* TODO: add tests for libexe_coff_header_read_data */

	/* TODO: add tests for libexe_coff_header_read_file_io_handle */
