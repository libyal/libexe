     ...
     const system_character_t *source )
{
	...
	if( source != NULL )
	{
		...
		/* Test libexe_check_file_signature with malloc failing in libbfio_file_initialize
		 */
     ...
     const system_character_t *source )
{
	...
	if( source != NULL )
	{
		...
		/* Test libexe_check_file_signature_wide with malloc failing in libbfio_file_initialize
		 */
		exe_test_malloc_attempts_before_fail = 0;
     ...
     const system_character_t *source )
{
	 ...
	 */
	result = exe_test_open_file_io_handle(
	          &file_io_handle,
	          empty_block,
	          sizeof( uint8_t ) * 1,
	          &error );
	...
	result = exe_test_close_file_io_handle(
	          &file_io_handle,
	          &error );
