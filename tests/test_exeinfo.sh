...
TEST_EXECUTABLE="../exetools/exeinfo";

if ! test -x "${TEST_EXECUTABLE}";
then
	TEST_EXECUTABLE="../exetools/exeinfo.exe";
fi
...
TEST_PROFILE_DIRECTORY=$(get_test_profile_directory "input" "exeinfo");
...
";
	...
	then
		...
		do
			run_test_on_input_file_with_options "${TEST_SET_DIRECTORY}" "exeinfo" "with_stdout_reference" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "${INPUT_FILE}" "${OPTIONS[@]}";
			RESULT=$?;
