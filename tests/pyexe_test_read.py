...
def pyexe_test_seek_offset_and_read_buffer(
    exe_section, input_offset, input_whence, input_size, expected_offset,
    expected_size):
...
def pyexe_test_read_buffer_at_offset(
    exe_section, input_offset, input_size, expected_offset, expected_size):
...
def pyexe_test_read(exe_section):
  ...
  if not pyexe_test_seek_offset_and_read_buffer(
      exe_section, 0, os.SEEK_SET, file_size, 0, file_size):
    return False
  ...
  if not pyexe_test_seek_offset_and_read_buffer(
      exe_section, file_size / 7, os.SEEK_SET, file_size / 2,
  ...
  if file_size < 1024:
    ...
    if not pyexe_test_seek_offset_and_read_buffer(
        exe_section, file_size - 1024, os.SEEK_SET, 4096, -1, -1):
      return False
    ...
    if not pyexe_test_seek_offset_and_read_buffer(
        exe_section, file_size - 1024, os.SEEK_SET, 4096,
        file_size - 1024, 1024):
      return False
  ...
  if not pyexe_test_read_buffer_at_offset(
      exe_section, file_size / 7, file_size / 2,
