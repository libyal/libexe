...
{
	static char *function = "libexe_array_initialize";
...
int libexe_array_free(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_array_free";
	int result            = 1;
...
int libexe_array_empty(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_array_empty";
...
int libexe_array_clear(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_array_clear";
...
int libexe_array_clone(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_array_clone";
	int entry_iterator    = 0;
     ...
     liberror_error_t **error )
{
	...
	static char *function = "libexe_array_resize";
	size_t entries_size   = 0;
	int entry_iterator    = 0;
	int result            = 1;
...
int libexe_array_get_number_of_entries(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_array_get_number_of_entries";
...
int libexe_array_get_entry_by_index(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_array_get_entry_by_index";
...
int libexe_array_set_entry_by_index(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_array_set_entry_by_index";
...
int libexe_array_append_entry(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_array_append_entry";
 ...
 * The entry_compare_function should return LIBEXE_ARRAY_COMPARE_LESS,
 * LIBEXE_ARRAY_COMPARE_EQUAL, LIBEXE_ARRAY_COMPARE_GREATER if successful or -1 on error
 *
 * Duplicate entries are allowed by default and inserted after the last duplicate entry.
 * Only allowing unique entries can be enforced by setting the flag LIBEXE_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES
 *
...
int libexe_array_insert_entry(
     ...
     liberror_error_t **error )
{
	static char *function = "libexe_tree_node_insert_node";
	int entry_iterator    = 0;
	int result            = -1;
	...
	if( ( insert_flags & ~( LIBEXE_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) ) != 0 )
	{
		liberror_error_set(
		 error,
		 LIBERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( array->entries != NULL )
	{
		     ...
		     entry_iterator++ )
		{
			...
			else if( result == LIBEXE_ARRAY_COMPARE_EQUAL )
			{
				if( ( insert_flags & LIBEXE_ARRAY_INSERT_FLAG_UNIQUE_ENTRIES ) != 0 )
				{
					return( 0 );
			...
			else if( result == LIBEXE_ARRAY_COMPARE_LESS )
			{
				break;
			}
			else if( result != LIBEXE_ARRAY_COMPARE_GREATER )
			{
				liberror_error_set(
				 error,
				 LIBERROR_ERROR_DOMAIN_ARGUMENTS,
	...
	if( ( array->entries != NULL )
	 && ( result == LIBEXE_ARRAY_COMPARE_LESS ) )
	{
