...
then
	docker run -t -v "${PWD}:/libexe" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libexe && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libexe;
