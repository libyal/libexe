...
void (*exetools_signal_signal_handler)( exetools_signal_t ) = NULL;

/* Signal handler for Ctrl+C or Ctrl+Break signals
 */
BOOL WINAPI exetools_signal_handler(
             exetools_signal_t signal )
{
	static char *function = "exetools_signal_handler";
	...
	switch( signal )
	{
		...
		case CTRL_C_EVENT:
			if( exetools_signal_signal_handler != NULL )
			{
				exetools_signal_signal_handler(
				 signal );
			}
			return( TRUE );
 ...
 */
void exetools_signal_initialize_memory_debug(
      void )
{
	int flag = 0;
 ...
 */
int exetools_signal_attach(
     void (*signal_handler)( exetools_signal_t ),
     libcerror_error_t **error )
{
	static char *function = "exetools_signal_attach";
	...
	exetools_signal_signal_handler = signal_handler;

	if( SetConsoleCtrlHandler(
	     (PHANDLER_ROUTINE) exetools_signal_handler,
	     TRUE ) == 0 )
	{
		libcerror_error_set(
		 error,
...
#if defined( _MSC_VER )
	exetools_signal_initialize_memory_debug();
#endif
 ...
 * Returns 1 if successful or -1 on error
 */
int exetools_signal_detach(
     libcerror_error_t **error )
{
	static char *function = "exetools_signal_detach";
	...
	exetools_signal_signal_handler = NULL;
